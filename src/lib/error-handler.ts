...
export function validatePagination(page?: string, limit?: string) {
  ...
  if (isNaN(pageNum) || pageNum < 1) {
    throw new ValidationError('Page must be a positive integer', { page: pageNum });
  }

  if (isNaN(limitNum) || limitNum < 1 || limitNum > 100) {
    throw new ValidationError('Limit must be between 1 and 100', { limit: limitNum });
  }
...
export function validateUUID(id: string, fieldName: string = 'ID'): string {
  ...
  if (!uuidRegex.test(id)) {
    throw new ValidationError(`Invalid ${fieldName} format`, { [fieldName.toLowerCase()]: id });
  }
